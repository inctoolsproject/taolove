...
mulai = time.time() 
dangerMessage = ["ควย","ไอ้สัส","เหี้ย","พ่องตาย"]
    ...
    try:
        ...
        if op.type == 5:
           ...
           if settings["autoAdd"] == True:
              ...
              if(settings["addmsg"]in[""," ","\n",None]):
                pass
              else:
                nadya.sendMessage(op.param1,str(settings["addmsg"]))
        if op.type == 5:
            print ("[ 5 ] AUTO BLOCK")
            if settings["autoBlock"] == True:
                nadya.blockContact(op.param1)
        if op.type == 17:
            print ("[ 17 ] WELCOME TO GROUP")
            group = nadya.getGroup(op.param1)
            if settings["welcome"] == True:
                nadya.sendMessage(op.param1,nadya.getContact(op.param2).displayName + settings["wcmsg"]+ str(group.name) + "\n\nขอบคุณที่สนับสนุนบอทจาก\nͲ̶̶̶̽Ꭺ̶̶̶̽Ꭼ̶̶̶̽Ꮇ̶̶̶̽ Ꭺ̶̶̶̽Ꮐ̶̶̶̽Ꭼ̶̶̶̽Ν̶̶̶̽Ͳ̶̶̶̽")
        ...
        if op.type == 15:
            print ("[ 15 ] OUT GROUP")
            group = nadya.getGroup(op.param1)
            if settings["welcomeout"] == True:
                nadya.sendMessage(op.param1,nadya.getContact(op.param2).displayName + settings["wcomsg"]+ str(group.name))
...
#==============================================================================#
                if text.lower() == 'help':
                    ...
                    nadya.sendContact(to, "ude3230559bf63a55b9c28aa20ea194e3")
...
#==============================================================================#
                    ...
                    start = time.time()
                    nadya.sendMessage(to, "̶T̶̶̶̶̶̶͓̽E̶̶̶̶̶̶͓̽A̶̶̶̶̶̶͓̽M̶̶̶̶̶̶͓̽ ̶̶N̶̶̶̶̶̶͓̽I̶̶̶̶̶̶͓̽G̶̶̶̶̶̶͓̽H̶̶̶̶̶̶͓̽T̶̶̶̶̶̶͓̽B̶̶̶̶̶̶͓̽A̶̶̶̶̶̶͓̽T̶̶̶̶̶͓̽")
                    elapsed_time = time.time() - start
                    nadya.sendMessage(to,format(str(elapsed_time)))
                elif text.lower() == 'creator':
                    nadya.sendMessage(to, "☣TEAM NIGHTBAT☣")
                    nadya.sendContact(to, "ude3230559bf63a55b9c28aa20ea194e3")
                elif "โทร" == msg.text.lower():
                    nadya.inviteIntoGroupCall(msg.to,[uid.mid for uid in cl.getGroup(msg.to).members if uid.mid != cl.getProfile().mid])
                    ...
                    nadya.sendMessage(to, "👍〘บอทยังอยู่ครับท่าน〙👍")
                elif "Phu2:" in msg.text:
                    bctxt = text.replace("Phu2:","")
                    t = nadya.getAllContactIds()
                    for manusia in t:
                        nadya.sendMessage(manusia,(bctxt))
                elif "Phu:" in msg.text:
                    bctxt = text.replace("Phu:","")
                    ...
                    if msg.toType == 2:
                        ...
                        try:
                            nadya.sendMessage(to, "บ้ายบาย~(｡•́︿•̀｡)")
                            nadya.leaveGroup(to)
                        except:
                            pass
                elif msg.text in ["เพื่อน"]:
                    anl = nadya.getAllContactIds()
                    ap = ""
                    for q in anl:
                        ap += "• "+cl.getContact(q).displayName + "\n"
                    nadya.sendMessage(msg.to,"「 รายชื่อเพื่อน 」\n"+ap+"จำนวน : "+str(len(anl)))
                elif "Vk " in msg.text:
                    ...
                    if targets == []:
                        ...
                        for target in targets:
                            if target in admin:
                                ...
                                try:
                                    nadya.kickoutFromGroup(to,[target])
                                except:
                                    pass
                elif text.lower() == 'zt':
                    gs = nadya.getGroup(to)
                ...
                if msg.text in dangerMessage:
                 ...
                 random.choice(Rfu).sendText(msg.to,"ตรวจพบคำหยาบ จำเป็นต้องนำสมาชิกนี้ออกเพื่อความวุ้นวายของสมาชิกในกลุ่ม (｀・ω・´)")		
                elif msg.text.lower() == "ออน":
                    nadya.sendMessage(msg.to,str(datetime.now() - start_runtime)[:-7].split(":")[0]+" ชั่วโมง, "+str(datetime.now() - start_runtime)[:-7].split(":")[1]+" นาที, "+str(datetime.now() - start_runtime)[:-7].split(":")[2]+" วินาที,")
                elif text.lower() == 'บอท':
                    try:
                        arr = []
                        ...
                        ret_ = "╔══[ เชลบอท ]"
                        ret_ += "\n╠ ไลน์ : {}".format(contact.displayName)
...
#==============================================================================#
                    ...
                    nadya.sendMessage(to, "ปิดแล้ว")
                elif text.lower() == 'wco on':
                    settings["welcomeout"] = True
                    nadya.sendMessage(to, "เปิดแล้ว!!")
                elif text.lower() == 'wco off':
                    settings["welcomeout"] = False
                    nadya.sendMessage(to, "ปิดแล้ว")
                    ...
                    nadya.sendMessage(msg.to,"เปิดส่งภาพคนแทคแล้ว")
                    ...
                    nadya.sendMessage(msg.to,"ปิดส่งภาพคนแทคแล้ว")

                elif msg.text in ["Respontag on","Tag on","My respon on","Respon:on"]:
                    settings["detectMention"] = True
                    nadya.sendMessage(msg.to,"เปิดตอบกลับแทคแล้ว")
                
                elif msg.text in ["Respontag off","Tag off","My respon off","Respon:off"]:
                    settings["detectMention"] = False
                    nadya.sendMessage(msg.to,"ปิดตอบกลับแทคแล้ว")
                elif text.lower() == 'copy':
                    settings["copy"] = True
                    nadya.sendMessage(to, "ลงข้อมูลผู้ติดต่อเพื่อคัดลอกบัญชี!!")
#==============================================================================#
                elif text.lower() == 'me':
                    ...
                    if 'MENTION' in msg.contentMetadata.keys()!= None:
                        ...
                        try:
                            ...
                            nadya.sendMessage(msg.to, "พบข้อผิดพลาด!!")
                    ...
                    try:
                        ...
                        nadya.sendMessage(msg.to, "พบข้อผิดพลาดร้ายแรง!!")
                    ...
                    autoDeny = -1
                    nadya.sendMessage(msg.to,"ตั้งค่าสำเร็จแล้ว")
...
#==============================================================================#
                    ...
                    for target in targets:
                        try:
                            ...
                            nadya.sendMessage(msg.to,"เพิ่มในรายการล้อเลียนแล้ว!!")
                            break
                        except:
                            nadya.sendMessage(msg.to,"พบข้อผิดพลาด!!")
                            break
                    ...
                    if mic == "on":
                        if settings["mimic"]["status"] == False:
                            ...
                            nadya.sendMessage(msg.to,"เริ่มการล้อเลียน")
                    elif mic == "off":
                        if settings["mimic"]["status"] == True:
                            settings["mimic"]["status"] = False
                            nadya.sendMessage(msg.to,"ปิดการล้อเลียน")
#==============================================================================#
                elif text.lower() == 'gc':
                    ...
                    settings["wcmsg"] = msg.text.replace("Update welcome:","")
                    nadya.sendMessage(msg.to,"update welcome message :"+ settings["wcmsg"]+"\n\n"+ datetime.today().strftime('%H:%M:%S'))
                elif msg.text in ["Check welcome"]:
                    if settings["lang"] == "JP":
                        nadya.sendMessage(msg.to,"yor bot message\n\n" + settings["wcmsg"])
                    else:
                        nadya.sendMessage(msg.to,"The automatic appending information is set as follows。\n\n" + settings["wcmsg"])
                
                elif "ตั้งแอด:" in msg.text:
                    settings["addmsg"] = msg.text.replace("Update add:","")
                    nadya.sendMessage(msg.to,"Update message auto add :"+ settings["addmsg"]+"\n\n"+ datetime.today().strftime('%H:%M:%S'))
                elif msg.text in ["Check add"]:
                    if settings["lang"] == "JP":
                        nadya.sendMessage(msg.to,"message auto add.\n\n" + settings["addmsg"])
                    else:
                        nadya.sendMessage(msg.to,"message add me.\n\n" + settings["addmsg"])
                        
                elif "ตั้งออก:" in msg.text:
                    settings["wcomsg"] = msg.text.replace("Update out:","")
                    nadya.sendMessage(msg.to,"Update message auto add :"+ settings["wcomsg"]+"\n\n"+ datetime.today().strftime('%H:%M:%S'))
                elif msg.text in ["Check out"]:
                    if settings["lang"] == "JP":
                        nadya.sendMessage(msg.to,"message auto welcone out group.\n\n" + settings["wcomsg"])
                    else:
                        nadya.sendMessage(msg.to,"message auto welcone out group.\n\n" + settings["wcomsg"])
...
#==============================================================================#          
                    ...
                    for a in range(k+1):
                        ...
                        nadya.sendMessage(to, "จำนวน {} คน".format(str(len(nama))))          
...
#==============================================================================#
        if op.type == 26:
            ...
            if msg.contentType == 0:
                if text is None:
                    return
            if msg.text in ["Tagall","แทค","แทก","Tag"]:
                    group = nadya.getGroup(msg.to)
                    nama = [contact.mid for contact in group.members]
                    k = len(nama)//100
                    for a in range(k+1):
                    ...
                    for mention in mentionees:
                        if nadyaMID in mention["M"]:
                            if settings["detectMention"] == True:
                                ...
                                nadya.sendMessage(to, "TEAM NIGHTBAT:BY=PHU")
                                sendMessageWithMention(to, contact.mid)
                            break
                   
                elif msg.text in ["ล็อกห้อง","ล็อคบ้าน","ชุดล็อค"]:
                        nadya.sendMessage(msg.to,"Set:iconlock:on")
                        nadya.sendMessage(msg.to,"Set:blockinvite:on")
                        nadya.sendMessage(msg.to,"Set:ownerlock:on")
                        nadya.sendMessage(msg.to,"Set:changenamelock:on")
                        nadya.sendMessage(msg.to,"Siriv10:招待URL拒否")
                elif msg.text in ["ปิดเชิญ"]:
                        nadya.sendMessage(msg.to,"Set:blockinvite:on")
                elif msg.text in ["เปิดเชิญ"]:
                        nadya.sendMessage(msg.to,"Set:blockinvite:off")
                elif msg.text in ["ล็อคหัวห้อง"]:
                        nadya.sendMessage(msg.to,"Set:ownerlock:on")
                elif msg.text in ["ล็อคชื่อ"]:
                        nadya.sendMessage(msg.to,"Set:changenamelock:on")
                elif msg.text in ["ล็อครูป"]:
                        nadya.sendMessage(msg.to,"Set:iconlock:on")
                elif msg.text in ["ตั้งค่า","เช็คตั้งค่า"]:
                        nadya.sendMessage(msg.to,"Set:check")
                elif msg.text in ["@","แอด","แอดมิน"]:
                        nadya.sendMessage(msg.to,"Siri:groupcreator")
                        nadya.sendMessage(msg.to,"แอดมินบอทSiri")
                elif msg.text in ["ดำ"]:
                        nadya.sendMessage(msg.to,"set:addblacklist")
                        nadya.sendMessage(msg.to,"ส่งContectเพื่อเพิ่มบัญชีดำ")
                elif msg.text in ["ขาว","แก้ดำ"]:
                        nadya.sendMessage(msg.to,"set:addwhitelist")
                        nadya.sendMessage(msg.to,"ส่งContectเพื่อเพิ่มบัญชีขาว")
                elif msg.text in ["เปิดลิ้ง"]:
                        nadya.sendMessage(msg.to,"Siri:inviteurl")
                elif msg.text in ["ปิดลิ้ง"]:
                        nadya.sendMessage(msg.to,"Siriv10:招待URL拒否")
                elif msg.text in ["ยกเลิกค้างเชิญ","ยกเชิญ"]:
                        nadya.sendMessage(msg.to,"Siriv10:cancelinvite(สั่ง2รอบครับ)")
                elif msg.text in ["ขอแอดมินใหม่"]:
                        nadya.sendMessage(msg.to,"set:changeowner")
                elif msg.text in ["ปลดหัวห้องว่าง"]:
                        nadya.sendMessage(msg.to,"Siriv10:forcerelease")
                        nadya.sendMessage(msg.to,"ใช้สำหรับแอดมินลบบัญชี")
                elif msg.text in ["เพิ่มแอดรอง","เพิ่มแอดลอง","เพิ่มแอด"]:
                        nadya.sendMessage(msg.to,"設定:予備作者変更")
                elif msg.text in ["สลับแอด","สลับ"]:
                        nadya.sendMessage(msg.to,"設定:作者交換")
                elif msg.text in ["@@","แอดสำลอง","แอดสำรอง"]:
                        nadya.sendMessage(msg.to,"siriv10:予備作成者")
                elif msg.text in ["บอทออก"]:
                        nadya.sendMessage(msg.to,"Siri:bye")
                elif msg.text in ["เรียกบอท","รี"]:
                        nadya.sendMessage(msg.to,"Siriv10:再招待")
                elif msg.text in ["เปลี่ยน@"]:
                        nadya.sendMessage(msg.to,"set:changeowner")
                elif msg.text in ["เปลี่ยน@@"]:
                        nadya.sendMessage(msg.to,"set:changeextracreator")
                elif msg.text in ["ลบ@@"]:
                        nadya.sendMessage(msg.to,"Set:deleteextracreator")
                elif msg.text in ["เช็คตั๋ว"]:
                        nadya.sendMessage(msg.to,"Siri:招待回数")
                elif msg.text in ["Help v10","คำสั่ง v10"]:
                        nadya.sendMessage(msg.to,"""􀬁􀄆heart eyes􏿿คำสั่งv10ใช้ง่ายๆ􀬁􀄊nervous laugh􏿿
    􀠁􀆨Merp􏿿คำสั่งจะอยู่น่าวงเล็บ􀠁��Merp􏿿

🎁ชุดล็อค[ชุดคำสั่งล็อกห้อง]

✒ปิดเชิญ[ป้องกันสมาชิกเชิญคน]

🎁เปิดเชิญ[ป้องกันสมาชิกเชิญคน]
...
🎁เปิดลิ้ง[เปิดลิ้งค์กลุ่ม]

✒ปิดลิ���ง[ป้���งกั���คนเ���ิดลิ���งค์]
